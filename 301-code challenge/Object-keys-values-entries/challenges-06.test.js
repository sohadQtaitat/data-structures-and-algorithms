...
const hasChildrenValues = (arr, character) => {
    for (let i = 0; i < arr.length; i++) {
        if (Object.values(arr)[i].name === character && (arr)[i].children.length > 0) {
            console.log(Object.values(arr)[i].name)
...
const hasChildrenEntries = (arr, character) => {
    for (let i = 0; i < arr.length; i++) {
        if (Object.entries(arr)[i][1].name === character && (arr)[i].children.length > 0) {
            return true;
        }
    };
    return false;
...
const totalCharacters = (arr) => {
    ...
    arr.forEach((val, idx) => {
        total++;
        if (Object.values(arr)[idx].spouse) {
            total++
        }
        total = total + Object.values(arr)[idx].children.length;
    });
    return total
};
...
const houseSize = (arr) => {
    ...
    arr.forEach((val, idx) => {
        ...
        total = total + Object.values(arr)[idx].children.length;
        sizes.push({ house: Object.values(arr)[idx].house, members: total });
    });
    return sizes;
};
...
const houseSurvivors = (arr) => {
    ...
    arr.forEach((val, idx) => {
        let total = 0;
        total++;
        if (deceasedSpouses.includes(Object.values(arr)[idx].spouse) === false && null) {
            total++
        }
        total = total + Object.values(arr)[idx].children.length;

        survivors.push({ house: Object.values(arr)[idx].house, members: total });
