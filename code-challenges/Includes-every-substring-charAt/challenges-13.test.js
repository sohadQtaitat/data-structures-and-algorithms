...
const allHappy = (arr) => !arr.map(value => value.includes(':)')).includes(false)
...
const findEvery = (arr, target) => !arr.map(value => value.includes(target)).includes(false)
...
const unenrollBrook = (arr) => arr.map(innerArr => innerArr.filter(value => !value.includes('Brook')))
